...
def search_startups(app, q: str = "", industry: str = "", status: str = "") -> list:
    """Search/filter startup projects with optional query, industry and status filters."""
    db = get_db(app)
    conditions = []
    params: list = []
    ...
    if q:
        conditions.append(
            "(LOWER(startup_name) LIKE ? OR LOWER(founder_name) LIKE ? "
            "OR LOWER(industry) LIKE ? OR LOWER(country) LIKE ? "
            "OR LOWER(idea_description) LIKE ?)"
        )
        like = f"%{q.lower()}%"
    ...
    rows = db.execute(
        f"SELECT * FROM startup_projects {where} ORDER BY created_at DESC",
        params,
    ).fetchall()
    return [dict(r) for r in rows]
...
def update_startup(app, project_id: int, data: dict) -> None:
    """Update all editable fields of a startup project."""
    db = get_db(app)
    db.execute(
        """
        UPDATE startup_projects
        SET startup_name = ?, founder_name = ?, country = ?, industry = ?,
            budget = ?, target_audience = ?, business_goal = ?,
            idea_description = ?, updated_at = datetime('now')
        WHERE id = ?
        """,
        (
            data["startup_name"],
...
def update_startup_status(app, project_id: int, status: str) -> None:
    ...
    db = get_db(app)
    db.execute(
        "UPDATE startup_projects SET status = ?, updated_at = datetime('now') WHERE id = ?",
        (status, project_id),
    )
    db.commit()
...
def delete_startup(app, project_id: int) -> None:
    ...
    db.commit()
    logger.info("Deleted startup project id=%s", project_id)


def get_startup_stats(app) -> dict:
    """Return aggregate counts used by the dashboard and analytics."""
    db = get_db(app)
    ...
    avg_viability = db.execute(
        "SELECT AVG(viability_score) FROM reports"
    ).fetchone()[0] or 0
    return {
        "total":         total,
        "analyzed":      analyzed,
        ...
        "avg_viability": round(avg_viability),
    }
